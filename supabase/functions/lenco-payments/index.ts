    ...
    switch (action) {
      case "initiate": {
        ...
        await supabase
          ...
          .update({
            ...
            status:
              collectionStatus === "successful" ? "successful" : collectionStatus === "failed" ? "failed" : "pending",
      ...
      case "verify": {
        ...
        if (!reference && !transaction_id) {
          return new Response(JSON.stringify({ error: "Reference or transaction_id required" }), {
            status: 400,
            headers: { ...corsHeaders, "Content-Type": "application/json" },
          });
        }
        ...
        if (transaction.lenco_transaction_id) {
          const lencoVerify = await fetch(`${LENCO_API_URL}/transactions/${transaction.lenco_transaction_id}`, {
            headers: {
              Authorization: `Bearer ${lencoApiToken}`,
            },
          });
      ...
      case "submit_otp": {
        ...
        if (!collection_id || !otp) {
          return new Response(JSON.stringify({ error: "collection_id and otp are required" }), {
            status: 400,
            headers: { ...corsHeaders, "Content-Type": "application/json" },
          });
        }
        ...
        if (!otpResponse.ok || !otpData) {
          return new Response(JSON.stringify({ error: "OTP verification failed", details: otpData }), {
            status: 400,
            headers: { ...corsHeaders, "Content-Type": "application/json" },
          });
        }
      ...
      case "request_payout": {
        ...
        if (accountError || !account) {
          return new Response(JSON.stringify({ error: "Payment account not found" }), {
            status: 404,
            headers: { ...corsHeaders, "Content-Type": "application/json" },
          });
        }
        ...
        if (txError) {
          ...
          return new Response(JSON.stringify({ error: "Failed to initiate payout" }), {
            status: 500,
            headers: { ...corsHeaders, "Content-Type": "application/json" },
          });
        }
