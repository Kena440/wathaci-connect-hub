-- Fix: avoid SECURITY DEFINER views while preventing PII exposure from profiles

-- 1) Restrict direct SELECT on profiles to a safe column subset
REVOKE ALL ON public.profiles FROM anon;
REVOKE ALL ON public.profiles FROM authenticated;

-- Allow authenticated users to read ONLY non-sensitive columns needed for public discovery
GRANT SELECT (
  id,
  display_name,
  account_type,
...
-- 2) Keep marketplace discovery via invoker-safe views (enforces RLS + column grants)
...
-- 3) Provide safe access to sensitive profile fields via controlled RPCs
-- Users: read their own full profile
...
-- Admins: list profiles with PII (still protected by has_role check)
CREATE OR REPLACE FUNCTION public.admin_list_profiles(p_limit int DEFAULT 50)
...
REVOKE ALL ON FUNCTION public.admin_list_profiles(int) FROM anon;
